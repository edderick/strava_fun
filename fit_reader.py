...
def process_file_header(fit_file):
    ...
    protocol_version, profile_version, data_size, data_type, crc = struct.unpack('<BHI4sH', header)
...
def process_data_message(fit_file, local_message_num):
    ...
    if PRINT_DATA:
        print(f'Message Type: {global_message_names[data_definition["global_message_num"]]}')
    for i, field in enumerate(data_definition['fields']):
        value = fit_file.read(field['field_size'])
        try: 
            unpacked, = struct.unpack(f'{data_definition["endian_modifier"]}{field["unpack_string"]}', value)
            if VERBOSE:
                print(f'field[{i}/{field["field_definition_number"]}] = {unpacked}')
        except: 
            if PRINT_DATA:
                print(f"Failed to unpack value: {value} as {field['base_type_name']} with {field['unpack_string']}. Size={field['field_size']}")


def process_field(fit_file, local_message_num):
    ...
    base_types = [
        ...
        'byte', 'sint64', 'uint64', 'unit64z'
    ]

    unpack_strings = [
        'b', 'b', 'B', 'h', 'H', 'i', 'I', f'{field_size}s', 'f', 'd', 'B', 'H', 'I', 'b', 'q', 'Q', 'Q'
    ]

    base_type_name = base_types[base_type_number]
    ...
    data_definitions[local_message_num]['fields'].append({
        ...
        'unpack_string': unpack_strings[base_type_number]
    })
...
def process_definition_message(fit_file, local_message_num, message_type_specific):
    ...
    if VERBOSE or PRINT_DEFINITIONS:
        print(f'Global Message: {global_message_names[global_message_num]} ({global_message_num})')
    ...
    for i in range(0, num_fields):
        process_field(fit_file, local_message_num)
...
with open('./_Mild_.fit', 'rb') as fit_file:
#with open('./Activity.fit', 'rb') as fit_file:
